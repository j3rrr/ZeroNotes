...
function ZNEventHandler:NewObject(f, name)
	local obj = {}
...
function ZNEventHandler:Register(event, f, name)
	...
	if not self.dtbl[event] then 
		self.dtbl[event] = {}
		ZNEventHandler:RegisterEvent(event)
	end
...
function ZNEventHandler:Unregister(event, name)
...
function ZNEventHandler:IsRegistered(event, name)
	local objs = self.dtbl[event]
...
function ZNEventHandler:GetRegisteredFunction(event, handler)
	local objs = self.dtbl[event]
	if not objs then return end
...
function ZNEventHandler:OnEvent(event, ...)
	local objs = self.dtbl[event]
	if not objs then return end
...
ZNEventHandler:SetScript('OnEvent', ZNEventHandler.OnEvent)